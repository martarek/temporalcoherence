...
%TODO  – Training procedure (one phase of training or training in several phases).
%TODO    – Description of the use of the network to make a prediction on new data.
...
\end{equation}
%TODO Removed b^l_p + because our computations does not include it.
...
\end{equation}
where $\delta$ is a margin that could be chosen as an hyperparameter. However, just as in~\cite{Mobahi2009}, the value of $\delta$ is fixed to 1.
...
\label{lst:algo}
\begin{itemize}
\item  \textbf{Inputs:} Labeled data ($x_n$, $y_n$), $n = 1, ...N$, unlabeled video data $x_n$,$n=N+1, ... N+U$ inputs are created from 
\item \textbf{while} Stopping criterion is not met \textbf{do}
\begin{enumerate}
\item Get a random labeled item ($x_n$,$y_n$)
\item Do a gradient step to minimize $L(\theta,x_n,y_n)$
\item Pick randomly two consecutive images ($x_m$,$x_n$) in the \textit{unlabeled} dataset (e.g video).
\item Do a gradient step to minimize $L_{coh1}(\theta,x_m,y_n)$
\item Pick randomly two images ($x_m$,$x_n$) in the \textit{unlabeled} dataset (e.g video).
\item Do a gradient step to minimize $L_{coh2}(\theta,x_m,y_n)$
\end{enumerate}
